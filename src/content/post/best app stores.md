...
It allows you to spoof your region and device model, allowing you to access that might not otherwise be available to you.
...
DietDroid's APK-dl is a website that like Aurora, downloads apps from Googles servers, but you don't need to install any store to use APK-dl.
Instead, you just download the APK to your phone, and can install the app directly, without any store.

APK-dl is the best option for downloading apps from Google Play without having to use an app store.
...
Izzyondroid is another repository of open source apps, but it has a larger selection of apps than F-Droid.

It can be accessed the same ways as F-Droid, through it's website and through F-Droid Clients.
